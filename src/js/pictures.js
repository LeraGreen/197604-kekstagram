function render() {
  ...
  function makePicturesArr() {
    var pictures = [];
    arrlikesCommentsPhotos.forEach(function(item ,i) {
      if (item['preview']) {
  ...
  function addPictureBlock() {
    ...
    var pictureBlock = templateContainer.querySelector('.picture').cloneNode(true);
    picturesContainer.appendChild(pictureBlock);
    });
  }
...
/*Как связать photoCard с img на странице и сделать, чтобы у каждого блока была разная ссылка на картинку, в этом мой вопрос*/
  /*function createImage() {
    ...
    photoCard.onload = function() {
      for (var i = 0; i < pictureCollection.length; i++) {
        pictureCollection[i].querySelector('img').
      }
    };
    photoCard..onerror = function() {
